  ...
  const handleSignUp = async (event) => {
    ...
    try {
      ...
      const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { company_name: cleanCompanyName, phone: cleanPhone }, // metadata
      ...
      const { error: profileError } = await supabase.from("profiles").insert({
        id: user.id,
        full_name: cleanCompanyName, // store company name here
        phone: cleanPhone,
        role: "customer",
      })
      ...
      if (profileError) {
        setDebug(
          JSON.stringify({ stage: "profiles.insert", error: profileError }, null, 2)
        )
